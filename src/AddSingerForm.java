 ...
 * @author Dell
 */
public class AddSingerForm extends javax.swing.JFrame {
    ...
    public AddSingerForm(int accountId){
        this();
        this.accountId = accountId;
    }
    
    public AddSingerForm(int accountId, Singer singer) {
        this(accountId);
        this.editMode = 1;
        addButton.setText("edit");
        topLabel.setText("Edit singer");
        
        this.singer = singer;
        this.person_image = singer.getPhoto();
                
        nameField.setText(this.singer.getName());
        ImageIcon imageIcon = new ImageIcon(new ImageIcon(this.singer.getPhoto()).getImage().getScaledInstance(lbl_image.getWidth(),lbl_image.getHeight(),Image.SCALE_SMOOTH));
    ...
    public void insertData(){
        ...
        if(!name.isEmpty() && person_image!=null){
            String sql = "INSERT INTO singer(name,photo,id_account) VALUES(?,?,?)";
            
            PreparedStatement pst = db.getPrepStmt(sql);
            try {
                pst.setString(1,name);
                pst.setBytes(2, this.person_image);
                pst.setInt(3,this.accountId);
                pst.executeUpdate();
            } catch (SQLException ex) {
                Logger.getLogger(AddSingerForm.class.getName()).log(Level.SEVERE, null, ex);
            }
            HomeForm singerForm = new HomeForm(this.accountId);
    ...
    public void updateData(){
        ...
        if(!name.isEmpty() && person_image!=null){
            ...
            String sql = "UPDATE singer SET name=?, photo=? WHERE id=?";
            
            PreparedStatement pst = db.getPrepStmt(sql);
            try {
                pst.setString(1,name);
                pst.setBytes(2, person_image);
                pst.setInt(3,this.singer.getId());
                pst.executeUpdate();
            } catch (SQLException ex) {
                Logger.getLogger(AddSingerForm.class.getName()).log(Level.SEVERE, null, ex);
            }
            HomeForm singerForm = new HomeForm(accountId);
            singerForm.setVisible(true);
    ...
    private void initComponents() {
        ...
        topLabel.setText("Add New Singer");
